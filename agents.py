...
class HR_Agents:
    def __init__(self):
        # Initialize language models with specified configurations
    ...
    def benchmark(self, role, inputs):
        return Agent(
            ...
            backstory=f"""You are an HR analyst with deep understanding on the {role} position.
            ...
            llm=self.OpenAIGPT35  # Consider if only this model is required here
        )
    ...
    def writer(self, role):
        return Agent(
            ...
            backstory=f"""You are an HR analyst specialized in writing job descriptions.
    ...
    def quality_assurance(self, role):
        return Agent(
            ...
            backstory=f"""You are a senior QA analyst specialized in reviewing job descriptions.
                You're responsible to ensure the job description for the {role} addresses the key requirements listed by the technical hiring manager.
                You ensure the tone reflects the company's culture, and you make sure the description is both detailed and easy to understand.
                Assess the results for grammar and factual accuracy.""",
